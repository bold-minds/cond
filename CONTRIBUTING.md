...
```bash
git clone https://github.com/bold-minds/cond.git
cd cond
go test ./...              # unit tests
...
```
cond/
├── cond.go                # Implementation (single file)
├── cond_test.go           # Unit tests
...
### Error Handling
- Base functions should not panic on ordinary inputs. `cond.If` is the deliberate exception — it panics on a mistyped branch argument because that's a programming error that should surface immediately. Document any new panic contract prominently.
...
- Package-level doc comment in `cond.go`.

### Dependencies
- **Zero external dependencies.** `cond` is pure stdlib.
...
`cond` is deliberately tiny (two functions). New additions are rare and must clear a high bar:
